...
description: "Audit the browser-enforced security primitives that every web app depends on: CORS (cross-origin resource sharing), Content Security Policy (CSP) including nonces and Trusted Types, cross-origin isolation (COOP / COEP / CORP), modern cookie security (SameSite, __Host- / __Secure- prefixes, CHIPS Partitioned cookies), Permissions-Policy, Subresource Integrity (SRI), HSTS, Referrer-Policy, iframe sandboxing, postMessage origin validation, Private Network Access (PNA), WebAuthn / Passkeys, and Federated Credential Management (FedCM). Sourced from web.dev, developer.chrome.com, the Fetch / HTML / Cookies / WebAuthn specs. Use whenever the user asks about browser security headers, CORS configuration, CSP design, secure cookies, cross-origin isolation, SharedArrayBuffer requirements, clickjacking, mixed content, HSTS preload, postMessage hardening, sandboxed iframes, passkeys, FedCM, Private Network Access, partitioned cookies, or 'audit my browser security'. Trigger on phrases like 'review my CSP', 'audit my CORS', 'are my cookies safe', 'enable cross-origin isolation', 'set up Trusted Types', 'WebAuthn integration', 'FedCM rollout', 'PNA preflight', 'CHIPS partitioned cookies'. Use this skill even when only one primitive is mentioned — coverage is broader than the trigger."
---
